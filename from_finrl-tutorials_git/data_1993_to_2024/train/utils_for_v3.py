...
def create_custom_baseline(data):
    """
    Create a custom baseline index from a DataFrame containing 'date' and 'close' columns for multiple tickers.
    """
    # Calculate average close price for each day
    custom_baseline = data.groupby('date')['close'].mean().reset_index()
    # Optionally normalize
    custom_baseline['normalized_close'] = custom_baseline['close'] / custom_baseline['close'].iloc[0] * 100
    return custom_baseline

# sample function call
# baseline_df_ = create_custom_baseline(processed)
# baseline_df = pd.DataFrame()
# baseline_df['date'] = baseline_df_['date']
# baseline_df['baseline'] = baseline_df_['normalized_close'] / baseline_df_['normalized_close'][0] * initial_amount
# baseline_df = baseline_df.set_index(baseline_df.columns[0])
# print(baseline_df)
...
def create_detailed_actions_excel(
    ...
    history_amount: Dict,
    history_total: Dict,
    turbulence_array: List,
    turbulence_bool: List,
    dates: List
):
    """
    ...
    - history_amount: Dictionary of amount of money left at the end of each market close
    - history_total: Dictionary of total amount of money + price_i*num_shares_i for every tic
    ...
    # Prepare the columns
    columns = ['date', 'day', 'turbulence', 'sell_all', 'funds_on_market_close', 'total'] + list(bought_tickers)
    ...
    for day, day_actions in history_action.items():
        ...
        # Add total money at market close
        row_data.append(history_total.get(day, np.nan))
