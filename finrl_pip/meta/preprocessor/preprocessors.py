...
class FeatureEngineer:
    ...
    def add_technical_indicator(self, data):
        ...
        # print(stock)
        # stock.to_csv("~/stockstats.csv", index=False)
        unique_ticker = stock.tic.unique()
        # print(f"TICKER: {unique_ticker}")

        for indicator in self.tech_indicator_list:
            indicator_df = pd.DataFrame()
            print(f"\n{indicator}")
            for i in tqdm(range(len(unique_ticker)), total=len(unique_ticker)):
                try:
                    temp_indicator = stock[stock.tic == unique_ticker[i]][indicator]
                    # print(f"{unique_ticker[i]}")
        ...
        # print(f"NEW DF")
        # print(df)
        fname = "/home/devmiftahul/trading_model/tuntun_data/Daily93Tuntun_2022_stockstats.csv"
